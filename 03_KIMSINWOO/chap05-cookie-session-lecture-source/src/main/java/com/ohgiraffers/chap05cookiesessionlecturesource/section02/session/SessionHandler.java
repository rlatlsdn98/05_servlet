...
public class SessionHandler extends HttpServlet {
    ...
    protected void doPost(HttpServletRequest request, HttpServletResponse response) throws ServletException, IOException {
        ...
        *   - 서버에서 해당 객체를 관리함
        *   - 생성된 Session객체의 ID를 클라이언트에게 전달(Cookie 이용)
        *   - 이후 요청 시 클라이언트의 쿠키(Session ID)를 같이 받아와 서버의 Session ID가 일치하는 Session객체가 있는지 확인
        *       -> 있으면 해당 객체를 반환해서 활용
        *           -> 이 때, 반환 받은 객체에 이전 값이 세팅되어 있다면 활용 가능 == 상태 유지
        *       -> 없으면 새 Session객체를 생성해서 반환
        * */
