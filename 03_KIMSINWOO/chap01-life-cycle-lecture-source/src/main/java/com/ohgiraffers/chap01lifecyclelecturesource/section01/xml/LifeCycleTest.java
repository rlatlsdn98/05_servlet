...
*
* Servlet의 생명 주기
*   - init() : Servlet 최초 호출 또는 서버 배포 시 해당 클래스를 인스턴스 화 하면서 실행
*   - service() :
*   - destroy() : 서버 재시작 또는 종료 시점에 서블릿 인스턴스 소멸하면서 실행되는 메서드
*
* Servlet 등록
* - HttpServlet을 상속 받은 클래스를 등록해서
*   Servlet Container가 관리하고, 요청/응답 처리를 할 수 있게 만드는 과정
* - xml 방식, 어노테이션 방식 두 가지가 있음
* */
public class LifeCycleTest extends HttpServlet {
    ...
    @Override
    public void destroy() {
        System.out.println("###### xml 매핑 destory() 메서드 호출 ######");
    }
}
