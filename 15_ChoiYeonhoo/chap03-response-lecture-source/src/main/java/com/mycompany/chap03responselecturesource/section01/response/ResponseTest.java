package com.mycompany.chap03responselecturesource.section01.response;
...
* 3. 응답을 처리한다.
*  => response를 이용해서 결과 웹을 만들고(동적 웹, HTML 코드)
*     스트림을 통해서 클라이언트에게 출력 */
...
//    resp.setContentType("text/html"); // 응답 데이터의 MIME Type을 설정
//    resp.setCharacterEncoding("UTF-8"); // 응답 데이터의 문자 셋 설정
    ...
    printWriter.print(sb.toString()); // html 코드 내보내기
    printWriter.flush();
    printWriter.close();
  }
}
